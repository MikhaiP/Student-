...
class CommunicationActivity : AppCompatActivity(), WifiDirectInterface, PeerListAdapterInterface, NetworkMessageInterface{
    ...
    override fun onPeerClicked(peer: WifiP2pDevice) {
        ...
        if(studentID.isNotBlank()){
            val hashedID = client?.hashStrSha256(studentID) ?:""
            val aesKey = client?.generateAESKey(hashedID)
            val aesIV = client?.generateIV(hashedID)

            if(aesKey != null && aesIV != null){
                val encryptedID = client?.encryptMessage(studentID,aesKey,aesIV)
                val content = ContentModel(studentID, deviceIp, studentID)
                client?.sendMessage(content)
